...
tropic_result tropic_init(void) {
  ...
  tropic_result result = tropic_handshake(tropic_secret_trezor_privkey,
                                          tropic_secret_tropic_pubkey);
...
inline tropic_result tropic_handshake(const uint8_t *trezor_privkey,
                                      const uint8_t *tropic_pubkey) {
  lt_ret_t ret = LT_FAIL;

  uint8_t trezor_pubkey[SECRET_TROPIC_KEY_LEN] = {};
  curve25519_scalarmult_basepoint(trezor_pubkey, trezor_privkey);

  ret = lt_session_start(&lt_handle, tropic_pubkey, PKEY_INDEX_BYTE,
